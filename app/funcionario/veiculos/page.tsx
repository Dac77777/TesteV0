...
export default function FuncionarioVeiculosPage() {
  ...
  const handleDeleteVehicle = (id) => {
    if (confirm("Tem certeza que deseja excluir este veículo?")) {
      ...
      activityLogger.log(
        "DELETE_VEHICLE",
        `Veículo excluído: ${vehicle?.plate} - ${vehicle?.brand} ${vehicle?.model}`,
        "VEICULOS",
      )
    }
  }
  ...
  const handleSaveVehicle = () => {
    if (isEditing) {
      ...
      activityLogger.log(
        "UPDATE_VEHICLE",
        `Veículo atualizado: ${currentVehicle.plate} - ${currentVehicle.brand} ${currentVehicle.model}`,
        "VEICULOS",
      )
    } else {
      ...
      activityLogger.log(
        "CREATE_VEHICLE",
        `Novo veículo criado: ${currentVehicle.plate} - ${currentVehicle.brand} ${currentVehicle.model}`,
        "VEICULOS",
      )
    }
  ...
  const handleTakePhoto = (vehicleId) => {
    ...
    setTimeout(() => {
      ...
      activityLogger.log(
        "TAKE_PHOTO",
        `Foto capturada do veículo: ${vehicle?.plate} - ${vehicle?.brand} ${vehicle?.model}`,
        "VEICULOS",
      )
    }, 1000)
  }
